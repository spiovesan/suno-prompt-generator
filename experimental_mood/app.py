...
if "_session_loaded" not in st.session_state:
    ...
    if working:
        ...
        for key in ["use_llm", "replace_guitar", "auto_fill_sections", "instrumental", "ensemble"]:
            if key in working:
...
with st.sidebar:
    ...
    instrumental = st.checkbox(
        ...
        value=st.session_state.pop("_loaded_instrumental", is_dev_focus),
        help="Add [Instrumental] tag and 'instrumental' markers to prevent Suno from generating vocals"
    )

    # Instrument ensemble — positive approach: define what you WANT
    ensemble = st.text_input(
        "Instrument Ensemble",
        value=st.session_state.pop("_loaded_ensemble", ""),
        key="ensemble_input",
        placeholder="e.g., guitar, piano, bass, drums",
        help="Define your instrument palette. Added to Style field and fills empty sections."
    )
...
if "loaded_song" in st.session_state:
    ...
    if "profile" in loaded:
        st.session_state["profile_select"] = loaded["profile"]
            ...
            with st.spinner(f"Generating{f' ({batch_idx+1}/{count})' if count > 1 else ''}..."):
                try:
                    ...
                    if batch_idx == 0:
                        ...
                        # Clear stale refinement outputs from previous generation
                        for stale_key in ["refined_style", "refined_lyrics",
                                          "refiner_reasoning", "style_score", "lyrics_score"]:
                        ...
                        # Auto-suggest a title if none set
                        if not st.session_state.get("suggested_title"):
                            api_key = st.session_state.get("openai_api_key", "")
                            st.session_state.suggested_title = suggest_song_title(
                                genre=current_genre,
                                mood=current_mood,
                                key=current_key,
                                mode=current_mode,
...
if "generated_style" in st.session_state:
    ...
    if "refined_style" in st.session_state:
        st.markdown("---")
        style_score = st.session_state.get("style_score", "?")
        lyrics_score = st.session_state.get("lyrics_score", "?")
        st.markdown(f"**Scores:** Style {style_score}/10 | Lyrics {lyrics_score}/10")
        ...
        with st.expander("🔍 Refinement Reasoning"):
            for step in st.session_state.get("refiner_reasoning", []):
                if "action" in step:
